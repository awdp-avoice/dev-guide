...
- Before running the script, check some of the `index.html` files in the browser, and copy the images you don't want in the output (like the 10MB gif) from `assets` to `/known-images-to-remove` in the repo (also check out `README.md` there). Those files are gitignored so that we don't commit unnecessary files in the repo.
